...
./prysmctl testnet generate-genesis --fork capella --num-validators 64 --genesis-time-delay 20 --config-name interop --chain-config-file config.yml --geth-genesis-json-in genesis.json  --geth-genesis-json-out genesis.json --output-ssz genesis.ssz --output-json genesis.ssz.json || exit 1

./geth --datadir=gethdata account import secret.txt || exit 1
